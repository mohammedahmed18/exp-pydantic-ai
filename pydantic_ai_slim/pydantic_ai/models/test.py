...
class TestModel(Model):
    ...
    def _get_tool_calls(self, model_request_parameters: ModelRequestParameters) -> list[tuple[str, ToolDefinition]]:
        """Return list of (tool_name, ToolDefinition). Optimized for both all-tools and subset modes."""
        function_tools = model_request_parameters.function_tools
        if self.call_tools == 'all':
            # Fast path: direct tuple construction using list comprehension
            return [(r.name, r) for r in function_tools]
        else:
            # Create mapping only once for fast indexing and avoid generator chain (build list directly)
