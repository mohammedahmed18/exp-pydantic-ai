...
class MistralStreamedResponse(StreamedResponse):
    ...
    def _validate_required_json_schema(json_dict: dict[str, Any], json_schema: dict[str, Any]) -> bool:
        ...
        if not required_params:
            return True  # Fast path for no required parameters

        properties = json_schema.get('properties')
        if not properties:
            # No defined properties, but has required - that's invalid schema, but keep old behavior:
            properties = {}

        # Cache mapping for repeated lookup
        type_map = VALID_JSON_TYPE_MAPPING

        # Convert repeated `json_dict.get(x)` & `properties.get(x, {})` to fast-path
        for param in required_params:
            ...
            if not param_schema:
                # No schema for param but it's required:
                continue

            param_type = param_schema.get('type')
            param_items_schema = param_schema.get('items')
            param_items_type = param_items_schema.get('type') if param_items_schema else None

            if param_type == 'array' and param_items_type:
                if not isinstance(param_val, list):
                    return False
                elem_type = type_map[param_items_type]
                # Avoid repeated lookups in loop
                if not all(isinstance(item, elem_type) for item in param_val):
                    return False
            elif param_type:
                # param_type is always in type_map (trusted schema)
                if not isinstance(param_val, type_map[param_type]):
                    return False

            # Only descend if this param is a dict and param_schema contains nested 'properties'
            if isinstance(param_val, dict) and 'properties' in param_schema:
                if not MistralStreamedResponse._validate_required_json_schema(
                    param_val, param_schema
                ):
                    return False
